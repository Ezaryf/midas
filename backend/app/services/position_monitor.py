...
<<<<<<< HEAD
        ...
        if pos.type == mt5.ORDER_TYPE_BUY and new_sl < pos.price_open:
            logger.debug(f"Trailing SL {new_sl:.2f} below entry {pos.price_open:.2f} — clamping to entry")
            new_sl = pos.price_open
        if pos.type == mt5.ORDER_TYPE_SELL and new_sl > pos.price_open:
...
class _NullPositionMonitor(PositionMonitor):
    ...
    def __init__(self):
        # Skip parent __init__ which logs config — we have no real config
        self.config = PositionMonitorConfig()
        self.magic_number = 0
        self.running = False
        self._task = None
